...
export default function Settings() {
  ...
  useEffect(() => {
    if (user) {
      ...
      if (user.avatar) {
        setAvatarPreview(`${API_URL.replace('/api', '')}${user.avatar}`)
      }
    }
  }, [user])
  ...
  const tabs = [
    { id: 'profile', label: { en: 'My Profile', ar: 'ملفي الشخصي' }, icon: 'M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z' },
  ]

  const initials = user ? `${user.first_name?.charAt(0) || ''}${user.last_name?.charAt(0) || ''}`.toUpperCase() : 'SA'
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600 dark:text-gray-400">
          {language === 'ar' ? 'إدارة النظام والإعدادات الشخصية' : 'Manage system and personal settings'}
        </p>
      </div>
      ...
      {activeTab === 'profile' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  ...
                  <p className="text-sm text-purple-500">{user?.role === 'owner' ? (language === 'ar' ? 'المالك' : 'Owner') : (language === 'ar' ? 'مدير النظام' : 'Super Admin')}</p>
                  <p className="text-xs text-gray-400 mt-1">{language === 'ar' ? 'انقر على الصورة للتغيير' : 'Click image to change'}</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{language === 'ar' ? 'الاسم الأول' : 'First Name'}</label>
                  <input 
                    type="text" 
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{language === 'ar' ? 'الاسم الأخير' : 'Last Name'}</label>
                  <input 
                    type="text" 
          ...
          <GlassCard className="p-6">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{language === 'ar' ? 'كلمة المرور الحالية' : 'Current Password'}</label>
                <input 
                  type="password" 
