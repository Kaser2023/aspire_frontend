...
import { initSocket, onNotificationCreated, offNotificationCreated } from '../../services/socket.service'
...
export default function Notifications() {
  ...
  const getNotificationColor = (type) => {
    switch (type) {
      ...
      case 'system_alert': return 'bg-purple-100 text-purple-600 dark:bg-purple-900/30 dark:text-purple-400'
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        {unreadCount > 0 && (
          <button 
            ...
            className="px-4 py-2 text-sm font-medium text-purple-600 dark:text-purple-400 hover:bg-purple-50 dark:hover:bg-purple-500/10 rounded-xl transition-colors"
          >
      ...
      {notifications.length > 0 ? (
        <div className="space-y-3">
          {notifications.map((notification) => (
            <GlassCard 
              key={notification.id} 
              className={`p-4 transition-all ${!notification.is_read ? 'border-l-4 border-purple-500' : ''}`}
            >
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      {!notification.is_read && (
                        <button 
                          ...
                          className="p-1.5 hover:bg-gray-100 dark:hover:bg-white/10 rounded-lg text-gray-400 hover:text-purple-500"
        ...
        <GlassCard className="p-8 text-center">
          ...
          <p className="text-sm text-gray-400">{language === 'ar' ? 'ستظهر الإشعارات هنا عند حدوث أنشطة جديدة' : 'Notifications will appear here when new activities occur'}</p>
        </GlassCard>
      )}
    </div>
  )
