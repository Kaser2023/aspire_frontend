      ...
      {notifications.length > 0 ? (
        <div className="space-y-3">
          {notifications.map((announcement) => {
            ...
            return (
              <GlassCard key={announcement.id} className="p-4 transition-all hover:shadow-md border-l-2 border-primary/70">
        ...
        <GlassCard className="p-8 text-center">
          ...
          <p className="text-gray-500 mb-2">{language === 'ar' ? 'لا توجد إشعارات' : 'No notifications'}</p>
          <p className="text-sm text-gray-400">{language === 'ar' ? 'ستظهر الإشعارات هنا عند وصولها' : 'Notifications will appear here when you receive them'}</p>
        </GlassCard>
      )}
    </div>
  )
